...
import task.resnet18 as resnet18
import task.common as util

TASK_NAME = 'resnet18_inference'
...
def import_func():
    def inference(model, data_b):
        print(threading.currentThread().getName(),
            'resnet18 inference >>>>>>>>>>', time.time(),
            'model status', model.training)
        with timer('resnet18 inference func'):
...
def import_task():
    model = import_model()
    func = import_func()
    group_list = resnet18.partition_model(model)
